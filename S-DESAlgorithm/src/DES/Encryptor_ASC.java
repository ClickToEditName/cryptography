...
public class Encryptor_ASC {
    ...
    private static String toStringASCII(String binary) {
        StringBuilder ascii = new StringBuilder();
        for (int i = 0; i < binary.length(); i += 8) {
            String byteString = binary.substring(i, Math.min(i + 8, binary.length()));
            int charCode = Integer.parseInt(byteString, 2);
            // 将可打印字符直接添加，其他字符以转义形式添加
            if (charCode >= 32 && charCode <= 126) {
                ascii.append((char) charCode);
            } else {
                ascii.append("\\u").append(String.format("%04x", charCode)); // 转义输出
            }
        }
